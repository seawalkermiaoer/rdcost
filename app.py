...
def check_credentials(username, password):
    ...
    try:
        ...
        logger.info(f"验证结果: {hashed_input == stored_password_hash}")
        logger.info(f"输入用户名: {username}, 存储用户名: {stored_username}")
        logger.info(f"输入密码哈希: {hashed_input}, 存储密码哈希: {stored_password_hash}")

        # 如果存储的是哈希密码
        if stored_password_hash:
            return username == stored_username and hashed_input == stored_password_hash
